...
if(isset($_POST['drug_name']))
{
	...
	$sql=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO tempPrescri(customer_id,customer_name,age,sex,postal_address,phone,drug_name,strength,dose,quantity)
	...
	$get_cost=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT cost FROM stock WHERE drug_name='{$drug}'");
	...
	echo "<table width=\"100%\" border=1>";
        ...
        // loop through results of database query, displaying them in the table
		 $result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tempPrescri")or die(mysqli_error($GLOBALS["___mysqli_ston"]));
...
$getDetails=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT invoice,drug,cost,quantity FROM invoice_details WHERE invoice='{$invoice}'");
$getQuantity=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT quantity FROM invoice_details WHERE invoice_id='{$invoice}'");
...
while($item5=mysqli_fetch_array($getDetails))
			{
			$getDrug=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT drug_name FROM stock WHERE stock_id='{$item5['drug']}'");
