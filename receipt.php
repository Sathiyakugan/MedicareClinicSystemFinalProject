...
$getCid=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT customer_id FROM prescription WHERE invoice_id='{$invNo}' ");
$getName=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT customer_name FROM invoice WHERE invoice_id='{$invNo}' ");
$details1=mysqli_fetch_array($getName); 
$details=mysqli_fetch_array($getCid);

	
	$sqlP=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO receipts(reciptNo,customer_id,total,payType,serialno,served_by)
