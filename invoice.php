...
$getPresid=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT 1+MAX(prescription_id) FROM prescription");
$presId=mysqli_fetch_array($getPresid);
		if($presId[0]=='')
		{$presIdd=999; }
		else{$presIdd=$presId[0];}
	
	$sqlP=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO prescription(prescription_id,customer_id,customer_name,age,sex,postal_address,invoice_id,phone)
				VALUES('{$presIdd}','{$c_id}','{$cname}','{$age}','{$sex}','{$postal}','{$invoiceNo}','{$phone}')  ");
		$sqlI=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO invoice(invoice_id, customer_name,served_by,status)
				VALUES('{$invoiceNo}','{$cname}','{$user}','Pending') ");
						
$getDetails=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM tempprescri WHERE customer_id='{$c_id}'");
while($item1=mysqli_fetch_array($getDetails))
			{	
				$getDetails1=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT stock_id, cost FROM stock WHERE drug_name='{$item1['drug_name']}'");	
			
				$details=mysqli_fetch_array($getDetails1);
				$sqlId=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO invoice_details(invoice,drug,cost,quantity)
...
while($item12=mysqli_fetch_array($getDetails))
			{	
			$getDetails12=mysqli_query($GLOBALS["___mysqli_ston"], "SELECT stock_id, cost FROM stock WHERE drug_name='{$item12['drug_name']}'");	
			
				$details2=mysqli_fetch_array($getDetails12);
			$sqlIp=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO prescription_details(pres_id,drug_name,strength,dose,quantity)
				VALUES('{$presIdd}','{$details2['stock_id']}','{$item12['strength']}','{$item12['dose']}','{$item12['quantity']}') ");	
					
			}
		$sqlD=mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM tempprescri WHERE customer_id='{$c_id}' ");
